...
interface UpdatedInfo {
  // Define aquí la estructura de updatedInfo según lo que envíes
  [key: string]: any;
}

export const GuardarInfoActualizada = async (updatedInfo: UpdatedInfo): Promise<any> => {
  try {
    ...
    const response = await fetch(API.DATA_REGISTER_UPDATE, { // Reemplaza por tu URL
      method: 'PUT',
      headers: {
